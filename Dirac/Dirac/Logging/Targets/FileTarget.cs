...
namespace Dirac.Logging
{
    ...
    public class FileTarget : LogTarget, IDisposable
    {
        ...
        public FileTarget(string fileName, Logger.Level minLevel, Logger.Level maxLevel, bool includeTimeStamps, bool reset = false)
        {
            ...
            this._filePath = string.Format("{0}/{1}", ServerLogConfig.Instance.Root, _fileName); // construct the full path using LoggingRoot defined in config.ini

            if (!Directory.Exists(ServerLogConfig.Instance.Root)) // create logging directory if it does not exist yet.
                Directory.CreateDirectory(ServerLogConfig.Instance.Root);
